...
- Reworked note create/edit server actions around explicit action state so form submissions can return field-level validation errors and preserve typed values instead of failing as full redirects, forced the app shell layout dynamic to avoid stale auth shell state, and deduplicated paginated note appends defensively in the client feed.
