...
- Added email confirmation auto-sign-in: `signUpAction` now sets `emailRedirectTo` so Supabase sends confirmation links pointing to `/auth/callback`, the new route handler exchanges the code for a session and calls `ensureProfileForUser`, then redirects to `/auth/confirmed` which auto-redirects to `/app` after 2 seconds; error cases redirect to `/auth/sign-in?error=...` which now renders an amber banner from the query param.
