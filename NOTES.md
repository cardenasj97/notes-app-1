...
- Hidden the add-member form in the app shell sidebar for `member`-role users so the visible UI matches the server-side permission model; admin and owner roles still see and can use the form.
- Implemented that sidebar permission fix in `app-shell.tsx` as a render guard around `AddMemberForm`, so `member` users no longer see an action they cannot complete while admin/owner behavior stays unchanged.
- Updated the note permission model so org members can edit org-visible and shared notes and can generate or accept AI summaries on those notes, while delete and sharing-management actions remain author-only. Brought README, QA acceptance coverage, note detail controls, and server-side permission helpers into alignment with that behavior.
