...
- `signUpAction` did not set `emailRedirectTo`, so Supabase used its dashboard default redirect, no code was exchanged, no session was created, and clicking the confirmation link left users with no session and no feedback. Fixed in this commit.
